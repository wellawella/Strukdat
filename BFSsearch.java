...
public class BFSsearch {
	...
	private static boolean checkRepeats(SearchNode n){
		...
		while (n.getParent() != null && !retValue){
			if (((SearchNode) n.getParent()).getCurState().equals(checkNode.getCurState()))
			{
				retValue = true;
			}
	...
	public static void performSearch(Queue<SearchNode> q, boolean d)
	{
		...
		while (!q.isEmpty()) // while the queue is not empty
		{
			...
			if (!((SearchNode) tempNode.getCurState()).isGoal()) // if tempNode is not the goal
													// state
			{
				ArrayList<State> tempSuccessors = (ArrayList<State>) ((SearchNode) tempNode.getCurState())
			...
			// it.
			{
				...
				tempNode = (SearchNode) tempNode.getParent();
				...
				for (int i = 0; i < loopSize; i++){
					...
					((SearchNode) tempNode.getCurState()).printState();
					System.out.println();
					System.out.println();
				}
				System.out.println("The cost was: " + tempNode.findCost());
				if (d) {
	...
	private static void genSuccessors() {
